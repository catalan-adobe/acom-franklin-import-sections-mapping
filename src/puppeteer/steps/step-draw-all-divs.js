  ...
  for (let i = 0; i < sections.length; i += 1) {
    ...
    if (!section.block && section.width > 0 && section.height > 0) {
      ...
      const svgBuffer = `<svg width="${section.width}" height="${section.height}">
          <rect width="${section.width - 8}" height="${section.height - 8}" x="4" y="4" fill="none" stroke="${strokeColor}" stroke-width="2"/>
        </svg>`;
      boxes.push({
...
function drawAllDivs({ outputFolder = `${process.cwd()}/divs`, exclusions = '' }) {
  ...
  return (action) => async (actionParams) => {
    ...
    try {
        ...
        if (!isCSSExcluded) {
          ...
          if (boundingBox) {
            ...
            if (
              ...
              && boundingBox.width * boundingBox.height > 380 * 260 // ~ card size
      ...
      await writeFileSync(pUtils.join(path, `${urlHash}-all-divs.json`), JSON.stringify(result, null, 2));
...
exports.drawAllDivs = drawAllDivs;
